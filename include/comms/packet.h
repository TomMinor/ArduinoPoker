...
///@brief
/// The packet protocol reads the payload length/type from the header file.
/// [<Type><Number of bytes>][<Byte0><Byte1>..<ByteN>]
struct Packet
{
  ///@brief Left nibble contains the packet content type,
  /// the right nibble contains the payload length
  uint8_t header;

  ///@brief The byte values to be sent across the serial connection
  uint8_t* payload;

  ///@brief How many bytes are to be sent across the serial connection
  uint8_t payloadSize;
  ...
  ///
  Packet(PacketType _type, uint8_t _size)
    : payloadSize(_size)
  {
