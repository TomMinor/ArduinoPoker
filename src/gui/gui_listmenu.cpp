#include "include/gui/gui_listmenu.h"

GUI::ListMenu::ListMenu(GUI::LabelFormat _format, std::vector<std::string> _strings, SDL_Point _origin) :
    m_currentSelected(0),
    m_origin(_origin)
...
void GUI::ListMenu::draw()
{
    //m_marker.draw();

    for (std::vector<GUI::Label>::iterator it = m_items.begin(); it != m_items.end(); ++it)
    {
        (*it).draw();
    }
}

void GUI::ListMenu::moveSelection(bool _isUp)
{
    int inc = _isUp ? -1 : +1;
